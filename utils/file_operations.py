...
def save_segments(top_segments, top_indices, boundaries, sample_rate, output_dir="../results"):
    ...
    for i, (segment, index) in enumerate(zip(top_segments, top_indices)):
        ...
        # Save audio file
        output_file = f"{output_dir}/match_{i+1}_segment_{index}_{start_time:.2f}s-{end_time:.2f}s.wav"
        torchaudio.save(output_file, segment, sample_rate=sample_rate)
        
        # Save metadata
        with open(f"{output_dir}/match_{i+1}_info.txt", "w") as f:
            f.write(f"Segment index: {index}\n")
            f.write(f"Time range: {start_time:.2f}s - {end_time:.2f}s\n")
